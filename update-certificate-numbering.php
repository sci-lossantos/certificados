<?php
// Actualizar la clase CertificateNumbering con el nuevo formato
...
class CertificateNumbering {
    ...
    /**
     * Obtiene todos los datos necesarios para el certificado
     */
    public function getCertificateData($documento_id) {
        $sql = "
            SELECT 
                d.*,
                p.nombres, p.apellidos, p.cedula,
                c.nombre as curso_nombre, c.duracion_horas, c.contenido_tematico,
                c.numero_registro_curso, c.lugar_realizacion, c.fecha_inicio, c.fecha_fin,
                e.nombre as escuela_nombre, e.id as escuela_id,
                cc.fecha_firma_certificados
            FROM documentos d
    ...
    /**
     * Formatea fechas para el certificado
     */
    public function formatearFecha($fecha, $formato = 'd/m/Y') {
        ...
        switch ($formato) {
            case 'texto':
                ...
                return $date->format('d') . ' de ' . $meses[(int)$date->format('n')] . ' de ' . $date->format('Y');
            default:
    ...
    /**
     * Formatea el intervalo de fechas
     */
    public function formatearIntervaloFechas($fecha_inicio, $fecha_fin) {
